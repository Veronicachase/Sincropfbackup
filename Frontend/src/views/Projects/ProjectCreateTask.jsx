...
import { Box, Grid, Select, MenuItem, TextField, Typography, Button, FormLabel } from "@mui/material";
    ...
    <Box>
      ...
      <Formik
        ...
        validationSchema={yup.object({
          ...
          assignedEmployee: yup.string().required("Assigning an employee is required")
        })}
        onSubmit={(values, actions) => {
          handleSubmitTask(values).then(() => {
      ...
      >
        {({ isSubmitting, setFieldValue, values }) => (
          <Form>
            <Box sx={{ flexGrow: 1, maxWidth: 800, margin: "2em auto", boxShadow: "shadow-custom", color: "#021F59" }}>
              <Grid container spacing={2}>
                <Grid item xs={12}>
                  <Field as={TextField} name="task" label="Task Name" fullWidth />
                </Grid>
                <Grid item xs={12}>
                  <Select
                    name="assignedEmployee"
                    value={values.assignedEmployee}
                    onChange={(event) => setFieldValue("assignedEmployee", event.target.value)}
                    fullWidth
                    displayEmpty
                  >
                    <MenuItem value="">
                ...
                <Grid item xs={12}>
                  <Field as={TextField} name="taskDescription" label="Task Description" multiline fullWidth />
                </Grid>
                <Grid item xs={12}>
                  {/* Assuming UploadImageCamera is a component you've defined to handle file uploads */}
                  <FormLabel>Add an Image</FormLabel>
                  {/* Replace `UploadImageCamera` with your actual file upload component */}
                </Grid>
                <Grid item xs={12}>
